...
function Library:CreateWindow(hubTitle)
    ...
    UIS.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            TS:Create(MainFrame, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    ...
    function WindowObj:AddToggle(name, defaultState, callback)
        ...
        SwitchBg.MouseButton1Click:Connect(function()
            state = not state
            TS:Create(Circle, TweenInfo.new(0.2), {Position = state and UDim2.new(1, -17, 0.5, -7) or UDim2.new(0, 3, 0.5, -7)}):Play()
            TS:Create(SwitchBg, TweenInfo.new(0.2), {BackgroundColor3 = state and C_ACCENT or C_OFF}):Play()
            TS:Create(Stroke, TweenInfo.new(0.2), {Color = state and C_ACCENT or C_BORDER}):Play()
